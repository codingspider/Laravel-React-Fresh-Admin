...
import { LIST_FLOOR, DELETE_FLOOR } from "../../routes/apiRoutes";
...
export default function FloorList() {
    ...
    const fetchData = useCallback(async () => {
        try {
            setIsLoading(true);
            const res = await api.get(LIST_FLOOR, {
                params: {
                    page: pageIndex + 1,
                    per_page: pageSize,
                    search: globalFilter || "",
    ...
    useEffect(() => {
        ...
        document.title = `${app_name} | Floor Management`;
        fetchData();
    }, [fetchData]);
    ...
    const deleteItem = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await api.delete(DELETE_FLOOR(id));
                toast({
                ...
                <Menu>
                    ...
                    <MenuList minW="140px" p={1.5}>
                        <MenuItem
                            ...
                            borderRadius="md"
                            fontSize="sm"
                            onClick={() => navigate(`/table-management/floor/edit/${row.original.id}`, { state: { floor: row.original } })}
                        >
                            {t("edit")}
                        </MenuItem>
                        <MenuItem
    ...
    return (
        <Box>
            <PageHeader
                title={t("floor_management")}
                subtitle={t("manage_all_floors")}
                breadcrumbs={[
                    { label: t("dashboard"), path: "/dashboard" },
                    { label: t("floors"), isCurrent: true },
                ]}
                action="/table-management/floor/create"
                actionLabel={t("add_floor")}
            >
                <TableExportButtons data={data} columns={columns} filename="floors" />
            </PageHeader>
