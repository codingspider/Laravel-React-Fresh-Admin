...
export default function MenuCategoryList() {
    ...
    const fetchData = useCallback(async () => {
        try {
            setIsLoading(true);
            const res = await api.get(LIST_MENU_CATEGORY, {
                params: {
                    page: pageIndex + 1,
                    per_page: pageSize,
                    search: globalFilter || "",
    ...
    useEffect(() => {
        ...
        document.title = `${app_name} | Menu Category Management`;
        fetchData();
    }, [fetchData]);
    ...
    const columns = [
        ...
        },
        {
            header: t("parent"),
            cell: ({ row }) => (
                <Text fontSize="sm">{row.original.parent?.name || "-"}</Text>
            ),
        },
        {
            header: t("items"),
                ...
                <Menu>
                    ...
                    <MenuList minW="140px" p={1.5}>
                        <MenuItem
                            ...
                            borderRadius="md"
                            fontSize="sm"
                            onClick={() => navigate(`/menu/category/edit/${row.original.id}`, { state: { category: row.original } })}
                        >
                            {t("edit")}
                        </MenuItem>
                        <MenuItem
    ...
    return (
        <Box>
            <PageHeader
                title={t("menu_category_management")}
                subtitle={t("manage_all_menu_categories")}
                breadcrumbs={[
                    { label: t("dashboard"), path: "/dashboard" },
                    { label: t("menu_categories"), isCurrent: true },
                ]}
                action="/menu/category/create"
                actionLabel={t("add_menu_category")}
            >
                <TableExportButtons data={data} columns={columns} filename="menu-categories" />
            </PageHeader>
