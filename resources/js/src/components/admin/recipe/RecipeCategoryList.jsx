...
export default function RecipeCategoryList() {
      ...
      cell: ({ row }) => (
        <Menu>
          ...
          <MenuList minW="140px" p={1.5}>
            <MenuItem icon={<Icon as={EditIcon} boxSize={4} />} borderRadius="md" fontSize="sm" onClick={() => navigate(RECIPE_CATEGORY_EDIT_PATH(row.original.id))}>
              {t("edit")}
            </MenuItem>
            <MenuItem
  ...
  return (
    <Box>
      <PageHeader
        title={t("recipe_categories")}
        subtitle={t("manage_recipe_categories")}
        breadcrumbs={[{ label: t("dashboard"), path: DASHBOARD_PATH }, { label: t("recipe_categories"), isCurrent: true }]}
        action={RECIPE_CATEGORY_ADD_PATH}
        actionLabel={t("add_category")}
      >
        <TableExportButtons data={data} columns={columns} filename="recipe-categories" />
      </PageHeader>
