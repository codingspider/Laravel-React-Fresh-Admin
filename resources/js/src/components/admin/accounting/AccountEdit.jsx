...
import { GET_ACCOUNT, UPDATE_ACCOUNT, POS_SETTINGS } from "../../../routes/apiRoutes";
...
const AccountEdit = () => {
    ...
    useEffect(() => {
        if (account) {
            ...
            setValue("opening_balance", account.opening_balance || 0);
            setValue("status", account.status === "active" ? "active" : "inactive");

            requestAnimationFrame(() => {
                setValue("account_group", account.account_group || "");
                setIsLoading(false);
            });
        }
    }, [account, setValue]);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const res = await api.put(UPDATE_ACCOUNT(id), payload);
            toast({
