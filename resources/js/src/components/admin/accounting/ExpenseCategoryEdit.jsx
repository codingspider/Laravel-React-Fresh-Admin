...
import { GET_EXPENSE_CATEGORY, UPDATE_EXPENSE_CATEGORY, LIST_ACCOUNT } from "../../../routes/apiRoutes";
import useThemeColors from "../../../hooks/useThemeColors";

const CATEGORY_LIST = "/accounting/expense-categories";
...
const ExpenseCategoryEdit = () => {
    ...
    useEffect(() => {
        const fetchAccounts = async () => {
            try {
                const res = await api.get(LIST_ACCOUNT);
    ...
    useEffect(() => {
        if (category) {
            ...
            setValue("account_id", category.account_id || "");
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const res = await api.put(UPDATE_EXPENSE_CATEGORY(id), data);
                ...
                <Card mb={4} bg={colors.bgCard} shadow="sm" borderRadius="lg" border="none">
                    <CardBody py={3}>
                        <Breadcrumb fontSize="sm" color={colors.textSecondary}>
                            ...
                            <BreadcrumbItem>
                                <BreadcrumbLink as={ReactRouterLink} to={CATEGORY_LIST} fontWeight="medium" _hover={{ color: "teal.500" }}>{t("expense_categories")}</BreadcrumbLink>
                            </BreadcrumbItem>
                ...
                <Card shadow="xl" borderRadius="xl" overflow="hidden" bg={colors.bgCard}>
                    <CardHeader bg={colors.bgCard} borderBottom="1px solid" borderColor={colors.borderSubtle} pb={6}>
                        <Flex justify="space-between" align="center">
                            <Box>
                                <Heading size="sm" color={colors.textPrimary} fontWeight="bold">{t("edit_expense_category")}</Heading>
                                <Text fontSize="sm" color={colors.textSecondary} mt={1}>{category?.name}</Text>
                            </Box>
                            <Button colorScheme="teal" as={ReactRouterLink} to={CATEGORY_LIST} variant="outline" display={{ base: "none", md: "inline-flex" }} size="sm" fontWeight="600">{t("list")}</Button>
                        </Flex>
                    </CardHeader>
                    ...
                    <CardBody p={8}>
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <SimpleGrid columns={{ base: 1, md: 2 }} spacing={8}>
                                ...
                                <FormControl>
                                    <FormLabel fontSize="sm" fontWeight="semibold" color={colors.textPrimary} mb={2}>{t("account_code")}</FormLabel>
                                    <Input
                                        {...register("code")}
                                        type="text"
                                ...
                                <FormControl>
                                    ...
                                    <HStack>
                                        <Switch {...register("status")} colorScheme="teal" value="active" />
                                        <Text fontSize="sm" color={colors.textSecondary}>{t("active")}</Text>
                                    </HStack>
                                </FormControl>
                            ...
                            <Flex mt={10} justify={{ base: "stretch", md: "flex-end" }} gap={4}>
                                <Button type="button" as={ReactRouterLink} to={CATEGORY_LIST} colorScheme="gray" variant="outline" fontWeight="semibold" px={6} h={12} borderRadius="md" w={{ base: "full", md: "auto" }} _hover={{ bg: "gray.50" }}>{t("cancel")}</Button>
