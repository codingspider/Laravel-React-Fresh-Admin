...
export default function ExpenseList() {
    ...
    const fetchData = useCallback(async () => {
        try {
            setIsLoading(true);
            const res = await api.get(LIST_EXPENSE, {
                params: {
                    page: pageIndex + 1,
                    per_page: pageSize,
                    search: globalFilter || "",
    ...
    const deleteItem = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await api.delete(`${DELETE_EXPENSE(id)}`);
                toast({
    ...
    const columns = [
        ...
        {
            header: t("expense_reference_number"),
        ...
        },
        {
            ...
            cell: ({ getValue }) => (
                <Text fontSize="sm">
                    {getValue()?.name || getValue()?.category || "-"}
                </Text>
            ),
        },
        {
                ...
                <Menu>
                    ...
                    <MenuList minW="140px" p={1.5}>
                        <MenuItem
                            ...
                            borderRadius="md"
                            fontSize="sm"
                            onClick={() => navigate(`/accounting/expenses/edit/${row.original.id}`, { state: { expense: row.original } })}
                        >
                            {t("edit")}
                        </MenuItem>
                        <MenuItem
    ...
    return (
        <Box>
            <PageHeader
                title={t("expenses")}
                subtitle={t("manage_all_expenses")}
                breadcrumbs={[
                    { label: t("dashboard"), path: "/dashboard" },
                    { label: t("expenses"), isCurrent: true },
                ]}
                action="/accounting/expenses/create"
                actionLabel={t("add_expense")}
            >
                <TableExportButtons data={data} columns={columns} filename="expenses" />
            </PageHeader>
