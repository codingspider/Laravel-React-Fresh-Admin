...
import { STORE_CURRENCY } from "../../../routes/apiRoutes";
...
const CurrencyCreate = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const res = await api.post(STORE_CURRENCY, data);
            toast({ title: res.data.message || t("currency_created"), status: "success", duration: 3000, isClosable: true });
            navigate(LIST_PATH);
        } catch (err) {
    ...
    useEffect(() => {
        ...
        document.title = `${app_name} | Currency Create`;
    }, []);
                    ...
                    <CardHeader bg={colors.bgCard} borderBottom="1px solid" borderColor={colors.borderSubtle} pb={6}>
                        <Flex justify="space-between" align="center">
                            <Box>
                                ...
                                <Text fontSize="sm" color={colors.textSecondary} mt={1}>{t("create_new_currency")}</Text>
                            </Box>
                            <Button
                                colorScheme="teal"
                                as={ReactRouterLink}
