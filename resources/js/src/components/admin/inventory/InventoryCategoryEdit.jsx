...
import { UPDATE_INVENTORY_CATEGORY, LIST_INVENTORY_CATEGORY } from "../../../routes/apiRoutes";
...
export default function InventoryCategoryEdit() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const res = await api.get(`${LIST_INVENTORY_CATEGORY}/${id}`);
        const cat = res.data.data;
        reset({ name: cat.name, description: cat.description || "", sort_order: cat.sort_order || 0, is_active: cat.is_active });
      } catch {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await api.put(UPDATE_INVENTORY_CATEGORY(id), data);
      toast({ title: res.data.message || t("inventory_category_updated"), status: "success", duration: 3000, isClosable: true });
      navigate(LIST_PATH);
    } catch (err) {
          ...
          <CardHeader bg={colors.bgCard} borderBottom="1px solid" borderColor={colors.borderSubtle} pb={6}>
            <Flex justify="space-between" align="center">
              <Box>
                ...
                <Text fontSize="sm" color={colors.textSecondary} mt={1}>{t("update_inventory_category_details")}</Text>
              </Box>
