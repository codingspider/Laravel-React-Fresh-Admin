...
const PurchaseEdit = () => {
  ...
  useEffect(() => {
    ...
    document.title = `${app_name} | ${t("purchase_edit")}`;
    Promise.all([
    ...
    }).catch(() => toast({ title: t("error"), description: t("failed_to_load_purchase"), status: "error", duration: 3000, isClosable: true }))
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await api.put(UPDATE_PURCHASE(id), {
        ...data,
        supplier_id: Number(data.supplier_id),
        branch_id: data.branch_id ? Number(data.branch_id) : null,
        items: itemsPayload,
      });
      toast({ title: res.data.message || t("purchase_updated"), status: "success", duration: 3000, isClosable: true });
        ...
        <Card shadow="xl" borderRadius="xl" overflow="hidden" bg={colors.bgCard}>
          <CardHeader bg={colors.bgCard} borderBottom="1px solid" borderColor={colors.borderSubtle} pb={6}>
            <Flex justify="space-between" align="center">
              <Box>
                <Heading size="sm" color={colors.textPrimary} fontWeight="bold">{t("edit_purchase")}</Heading>
                <Text fontSize="sm" color={colors.textSecondary} mt={1}>{t("update_purchase")}</Text>
              </Box>
