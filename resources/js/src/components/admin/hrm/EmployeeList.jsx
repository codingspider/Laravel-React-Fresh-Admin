...
import {
  ...
  HRM_EMPLOYEE_CREATE_PATH,
  HRM_EMPLOYEE_VIEW_PATH,
  DASHBOARD_PATH,
...
export default function EmployeeList() {
  ...
  useEffect(() => {
    ...
    document.title = `${app_name} | ${t("employees")}`;
    fetchData();
  ...
  const deleteItem = async (id) => {
    ...
    try {
      const res = await api.delete(DELETE_EMPLOYEE(id));
      toast({ title: res.data.message || t("employee_deleted"), status: "success", duration: 3000, isClosable: true });
      fetchData();
    } catch (err) {
      toast({ title: t("error"), status: "error", duration: 3000, isClosable: true });
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Box>
          <Text fontSize="sm" fontWeight="600">{row.original.full_name || row.original.name}</Text>
          <Text fontSize="xs" color={colors.textMuted}>{row.original.email}</Text>
        </Box>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        const val = getValue();
        return (
          <Badge colorScheme={val === "active" ? "green" : val === "inactive" ? "orange" : "red"} variant="subtle" borderRadius="full" px={2} py={0.5} fontSize="xs">
            {val || "inactive"}
          </Badge>
        );
      },
      ...
      cell: ({ row }) => (
        <Menu>
          ...
          <MenuList minW="180px" p={1.5}>
            <MenuItem icon={<Icon as={Eye} boxSize={4} />} borderRadius="md" fontSize="sm" onClick={() => navigate(HRM_EMPLOYEE_VIEW_PATH(row.original.id))}>
              {t("view")}
            </MenuItem>
            <MenuItem icon={<Icon as={Edit} boxSize={4} />} borderRadius="md" fontSize="sm" onClick={() => navigate(`/hrm/employee/edit/${row.original.id}`)}>
              {t("edit")}
            </MenuItem>
  ...
  return (
    <Box>
      <PageHeader
        title={t("employees")}
        subtitle={t("manage_employees")}
        breadcrumbs={[{ label: t("dashboard"), path: DASHBOARD_PATH }, { label: t("employees"), isCurrent: true }]}
        action={HRM_EMPLOYEE_CREATE_PATH}
        actionLabel={t("add_employee")}
      >
        <TableExportButtons data={data} columns={columns} filename="employees" />
      </PageHeader>

      <SimpleGrid columns={{ base: 2, md: 4 }} spacing={4} mb={4}>
        <Box bg={colors.bgCard} p={5} borderRadius="xl" boxShadow="card" border="1px solid" borderColor={colors.borderDefault}>
          <Text fontSize="xs" color={colors.textSecondary} fontWeight="600">{t("total_employees")}</Text>
          <Heading size="lg" color="teal.500" mt={1}>{summary.total_employees || 0}</Heading>
        </Box>
      </SimpleGrid>
