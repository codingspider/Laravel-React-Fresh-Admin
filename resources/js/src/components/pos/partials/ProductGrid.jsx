...
import {
  ...
  AspectRatio, Center, useColorModeValue, Button, Spinner,
...
export default function ProductGrid({
  filteredItems, addToCart, isLoading = false, hasMore = false,
  isLoadingMore = false, onLoadMore, totalItems = 0, loadedCount = 0,
}) {
      ...
      {isLoading && filteredItems.length === 0 ? (
        <Center h="100%">
          <VStack spacing={3}>
            ...
            <Text color={colors.textSecondary} fontSize="sm">{t('Loading products...')}</Text>
          </VStack>
        </Center>
        ...
        <>
          ...
          {hasMore && (
            <Center py={4}>
              ...
              >
                {t('Load More')}
                {totalItems > 0 && ` (${t('Showing')} ${loadedCount} ${t('of')} ${totalItems})`}
              </Button>
            </Center>
          )}
        </>
